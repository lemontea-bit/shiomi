...
export function FieldSwitcher({ field, onPick }: { field: Field; onPick: (f: Field) => void }) {
  return (
    <div style={{ flex: 'none', padding: '4px 20px 12px' }}>
      <div style={{ display: 'flex', gap: 3, padding: 3, borderRadius: 12, background: 'rgba(255,255,255,.05)', border: '1px solid rgba(140,190,200,.12)' }}>
        {ORDER.map((k) => {
          const on = field === k;
          return (
            <button
              ...
              style={{
                ...
                background: on ? 'var(--amber)' : 'transparent', color: on ? '#0B1418' : 'rgba(234,242,244,.55)',
