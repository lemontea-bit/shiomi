...
export function NowWeatherGrid({
  ...
  return (
    <div style={{ borderRadius: 18, background: '#FFFBF4', border: '1px solid rgba(43,32,22,.12)', padding: '16px 16px 14px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
          <div style={{ font: "700 13px/1 'Zen Kaku Gothic New',sans-serif" }}>現在の天気</div>
          {source === 'simulated' && <div style={{ font: "500 9px/1 'JetBrains Mono',monospace", color: 'rgba(43,32,22,.35)' }}>推定値（オフライン）</div>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <WxIcon kind={weatherKind} size={26} />
          <div style={{ font: "400 11px/1 'Zen Kaku Gothic New',sans-serif", color: 'rgba(43,32,22,.55)' }}>{weatherLabel}</div>
        </div>
      </div>
        ...
        {stats.map((n) => {
          ...
          return (
            <div key={n.label} style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 0 }}>
              <div style={{ font: "400 10px/1 'JetBrains Mono',monospace", color: 'rgba(43,32,22,.42)', letterSpacing: '.08em' }}>{n.label}</div>
              <div
              ...
              >
                <div style={{ font: "700 20px/1 'JetBrains Mono',monospace", color: '#2B2016' }}>{n.value}</div>
                <div style={{ font: "400 10px/1 'JetBrains Mono',monospace", color: 'rgba(43,32,22,.45)' }}>{n.unit}</div>
              </div>
